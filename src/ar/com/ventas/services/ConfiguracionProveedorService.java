...
public class ConfiguracionProveedorService {
    
    public ConfiguracionProveedor getFacturas(Long id) throws Exception{
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        ConfiguracionProveedor facturas = null;
        try {
            facturas = new ConfiguracionProveedorBO().getFacturasById(id);
            tx.commit();
        }catch (Exception ex) {
            tx.rollback();
    ...
    public void saveConfiguracion(ConfiguracionProveedor conf) throws Exception{
        
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        try {
            new ConfiguracionProveedorBO().saveConfiguracion(conf);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public void updateConfiguracion(ConfiguracionProveedor config) throws Exception {
        ...
        Transaction tx = session.beginTransaction();
        try {
            new ConfiguracionProveedorBO().updateConfiguracion(config);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
