 ...
 */
public class ConvertirComprobanteComprasFrame extends javax.swing.JFrame {
     ...
     */
    public ConvertirComprobanteComprasFrame(ComprobanteCompras comprobante, Date df, Date af, Integer row) {
    ...
    private void guardarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBtnActionPerformed
        int a = JOptionPane.showConfirmDialog(this, "CONFIRME CONVERTIR DOCUMENTO, NO SE PUEDE REVERTIR", "Atención", JOptionPane.YES_NO_OPTION);
        if (a == 0) {
            guardar();
        }
...
//                              request focus mismo campo
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ConvertirComprobanteComprasFrame(null, null, null, 0).setVisible(true);
            }
        });
    }
    ...
    private void guardar() {
        if (validar()) {
            ...
            ComprobanteCompras cc_ex = comprobante;
            ...
            comprobante.setProveedor(cc_ex.getProveedor());
...
//            int a = JOptionPane.showConfirmDialog(this, "Confirma CONVERTIR el Comprobante?",
...
//            }
            ...
            try {
                ...
                Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this, "No se pudo actualizar comprobante");
                return;
            }
            s = proveedorSeleccionado.getSaldo();
            s += cc_ex.getTotal() - comprobante.getTotal();
            proveedorSeleccionado.setSaldo(s);
            try {
                new ProveedorService().updateProveedor(proveedorSeleccionado);
            } catch (Exception ex) {
                Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this, "No se pudo actualizar PROVEEDOR");
                return;
            }
            id = comprobante.getId();
            if(cc_ex.getTieneDetalle() != null){
                if(cc_ex.getTieneDetalle()){
                    List<ComprobanteComprasDetalle> ccde = null;
                    try {
                        ccde = new ComprobanteComprasDetalleService().getDetalleByComprobante(cc_ex);
                    } catch (Exception ex) {
                        Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    if(ccde != null && !ccde.isEmpty()){
                        for(ComprobanteComprasDetalle compD:ccde){
                            try {
                                new ComprobanteComprasDetalleService().deleteComprobanteDetalle(compD);
                            } catch (Exception ex) {
                                Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                                JOptionPane.showMessageDialog(this, "No se pudo actualizar DETALLES DE COMPROBANTE");
                            }
                        }
                    }
                }
            ...
            try {
                sc = new SeguimientoComprasService().getByComprobante(cc_ex);
                new SeguimientoComprasService().deleteSeguimientoCompras(sc);
            } catch (Exception ex) {
                Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this, "No se pudo actualizar SEGUIMIENTO");
                return;
            }
            ...
            try {
                ...
                Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this, "No se pudo actualizar CUENTA CORRIENTE");
                return;
            }
            try {
                new ComprobanteComprasService().deleteComprobante(cc_ex);
            } catch (Exception ex) {
                Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this, "NO SE BORRO EL ORIGINAL");
                return;
            }
            ccp = new CuentaCorrienteProveedor();
            ...
            if (tipoComp < 4) {
                ...
                try {
                    ...
                    Logger.getLogger(ConvertirComprobanteComprasFrame.class.getName()).log(Level.SEVERE, null, ex);
                    JOptionPane.showMessageDialog(this, "No se pudo actualizar IVA COMPRAS");
                    return;
                }
            }
            JOptionPane.showMessageDialog(this, "Comprobante guardado correctamente");
            AbmComprobantesCompraFrame accf = new AbmComprobantesCompraFrame(fd1, fa1, row1);
...
//        if (pj.printDialog()) {
    ...
    class MyPrintable implements Printable {
        ...
        public int print(Graphics g, PageFormat pf, int pageIndex) {
            ...
            g2.drawString(totalTxt.getText(), 380, row);
            row = 455;
            g2.setFont(new Font("Monospaced", Font.PLAIN, 14));
            g2.drawString(espacio + "X", 30, row);
            g2.setFont(new Font("Monospaced", Font.PLAIN, 8));
            ...
            row += 15;
            g2.drawString(detalle4Txt.getText(), 50, row);
            g2.drawString(totalTxt.getText(), 480, row);
            return PAGE_EXISTS;
        }
    }
}
